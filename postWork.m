...
for jj = 3:length(listOfRes)
    ...
    poligon(length(poligon(:,1)) + 1,1) = file.K_fastCircle_psd;
    poligon(length(poligon(:,1))    ,2) = file.K_slowWindow;
    poligon(length(poligon(:,1))    ,3) = 0;
    file.pass = 0;
...
%     corruptErrorType( jj - 2 )  = file.corruptErrorType;
%     if corruptErrorType(jj - 2) == 1
%           corruptPSDError_step( jj-2 )   = file.corruptPSDError;
%     elseif corruptErrorType(    jj - 2 ) == 2
%           corruptPSDError_ramp( jj-2 )   = file.corruptPSDError;
%     end
%     corruptSatCount(  jj - 2 )  = file.corruptSatCount;
% 
%     crds(:,jj-2) = file.start_position;
